...
| [`waitForConnections`](#parameter-waitforconnections) | bool | When true, the deployment will pause for a fixed, predetermined number of seconds before continuing; set to false to skip the wait. |
| [`waitForProject`](#parameter-waitforproject) | bool | When true, the deployment will pause for a fixed, predetermined number of seconds before proceeding; set to false to continue immediately without waiting. |
